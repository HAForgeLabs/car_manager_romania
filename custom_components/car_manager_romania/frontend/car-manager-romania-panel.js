class CarManagerRomaniaPanel extends HTMLElement {
  ...
  _buildVehicles() {
    ...
    for (const [entityId, stateObj] of Object.entries(states)) {
      ...
      vehicles.push({
        ...
        label: attrs.name || attrs.license_plate || attrs.vin || this._shortLabel(attrs.friendly_name || "Autovehicul"),
        plate: attrs.license_plate || "",
        vin: attrs.vin || "",
        status: stateObj.state || "—",
        attrs,
        entities: [{ entityId, stateObj, registry: this._hass?.entities?.[entityId] || {} }],
        statistics: attrs.vehicle_statistics || {},
        charts: attrs.vehicle_chart_data || {},
        service_history: Array.isArray(attrs.service_history) ? attrs.service_history : [],
        fuel_receipts: Array.isArray(attrs.fuel_receipts) ? attrs.fuel_receipts : [],
        fuel_intervals: Array.isArray(attrs.fuel_consumption_intervals) ? attrs.fuel_consumption_intervals : [],
        tire_sets: Array.isArray(attrs.tire_sets) ? attrs.tire_sets : [],
        equipment_items: Array.isArray(attrs.equipment_items) ? attrs.equipment_items : [],
        battery_items: Array.isArray(attrs.battery_items) ? attrs.battery_items : [],
        current_battery: attrs.current_battery || null,
        summary: attrs,
      });
    }
    ...
    return `
      <article class="cmr-vehicle-card ${statusClass}">
        ...
        <div class="cmr-mini-grid">
          ...
          <div><span>Combustibil an</span><strong>${this._formatMoney(fuel)}</strong></div>
          <div><span>Service an</span><strong>${this._formatMoney(service)}</strong></div>
        </div>
        ...
        <footer>
          <button data-tab="statistics" data-vehicle="${this._escape(vehicle.vehicle_id)}">Statistici</button>
          <button data-tab="fuel" data-vehicle="${this._escape(vehicle.vehicle_id)}">Combustibil</button>
          <button data-tab="costs" data-vehicle="${this._escape(vehicle.vehicle_id)}">Costuri</button>
        </footer>
      </article>
    `;
  }
    ...
    return `
      <main class="cmr-page cmr-settings-page">
        ...
        <section class="cmr-settings-hero">
          ...
          ${this._settingsHeroTile("Versiune", "1.0.72b55", "panel nou Car Manager", "mdi:package-variant-closed-check")}
        </section>
        ...
        <section class="cmr-settings-grid">
          <article class="cmr-settings-card">
            ...
            <div class="cmr-settings-shortcuts">
              ...
              <button data-tab="fuel"><ha-icon icon="mdi:gas-station"></ha-icon> Bonuri combustibil</button>
              <button data-tab="tires"><ha-icon icon="mdi:tire"></ha-icon> Anvelope</button>
              <button data-tab="equipment"><ha-icon icon="mdi:shield-car"></ha-icon> Dotări</button>
              <button data-tab="battery"><ha-icon icon="mdi:car-battery"></ha-icon> Baterie</button>
          ...
          <article class="cmr-settings-card">
            ...
            <div class="cmr-settings-steps">
              ...
              <div><strong>2</strong><span>Actualizează resursa Lovelace la <code>?v=1.0.72b55</code>.</span></div>
    ...
    return `
      <main class="cmr-page cmr-costs-page">
        ...
        <section class="cmr-costs-section">
          ...
          <div class="cmr-cost-vehicle-grid">
  ...
  _costMini(label, value) {
    return `<div><span>${this._escape(label)}</span><strong>${this._formatMoney(value, 2)}</strong></div>`;
  }
    ...
    return `
      <article class="cmr-admin-vehicle-card ${statusClass}">
        ...
        <div class="cmr-admin-sections">
          ...
          ${this._renderAdminFuelDetails(vehicle)}
          ${this._renderAdminUsageStats(vehicle)}
          ${this._renderAdminConsumables(vehicle)}
  ...
  _styles() {
    return `
      ...
      *{box-sizing:border-box}
      ...
      .cmr-tabs{position:sticky;top:8px;z-index:5;display:grid;grid-template-columns:repeat(10,minmax(0,1fr));gap:6px;background:rgba(255,255,255,.92);backdrop-filter:blur(14px);border:1px solid rgba(15,63,94,.08);border-radius:26px;padding:10px;margin-bottom:18px;box-shadow:0 18px 45px rgba(8,52,79,.11)}.cmr-tab{min-width:0;border:0;background:transparent;color:#4f5963;border-radius:18px;padding:11px 5px;display:flex;align-items:center;justify-content:center;gap:5px;font-size:13px;font-weight:850;cursor:pointer;white-space:nowrap}.cmr-tab ha-icon{width:20px;height:20px;flex:0 0 auto}.cmr-tab span{overflow:hidden;text-overflow:ellipsis}.cmr-tab.active{background:linear-gradient(135deg,#0b5b82,#19acc0);color:#fff;box-shadow:0 14px 28px rgba(9,98,134,.25)}
      ...
      @media (prefers-color-scheme: dark){
        ...
        .cmr-settings-field input,.cmr-fuel-form-grid input,.cmr-fuel-form-grid select,.cmr-fuel-form-grid textarea,
        ...
        .cmr-settings-field input:focus,.cmr-fuel-form-grid input:focus,.cmr-fuel-form-grid select:focus,.cmr-fuel-form-grid textarea:focus,
